...
export function removeUnsupportedBrowserMethods(
  ...
  return nodes
    .filter((path: ASTPath<CodeModNode>) => {
      const identifier: CodeModNode | null = getIdentifier(path.node.callee)
...
export function transformBrowserMethods(j: JSCodeshift, nodes: Collection<CodeModNode>): Collection<CodeModNode> {
  return nodes
    ...
    .forEach((path: ASTPath<CodeModNode>) => {
      ...
      const method: string = node.callee?.property?.name as string
      ...
      if (method === 'perform') {
        ...
        const transformedArg =
          (node.callee?.object?.arguments as CodeModNode[])[0]?.callee &&
          replaceCySelector(
            j,
            (node.callee?.object?.arguments as CodeModNode[])[0],
            'get',
            (node.callee?.object?.arguments as CodeModNode[])[0]?.callee?.property?.name,
          )
          ...
          j.callExpression(
            j.memberExpression(
              j.callExpression(j.memberExpression(j.identifier('cy'), j.identifier('get'), false), [
                (node.callee?.object?.arguments as CodeModNode[])[0]?.name
                  ? j.identifier((node.callee?.object?.arguments as CodeModNode[])[0]?.name as string)
                  : j.stringLiteral(
                      transformedArg && transformedArg.arguments[0]
                        ? ((transformedArg?.arguments as CodeModNode[])[0].value as string)
                        : (((node.callee?.object?.arguments as CodeModNode[])[0]?.arguments as CodeModNode[])[0]
                            ?.value as string),
                    ),
              ]),
              j.identifier(
  ...
  return root
    ...
    .replaceWith((path: ASTPath<CallExpression>) =>
      j.callExpression(j.memberExpression(j.identifier('cy'), j.identifier('go'), false), [
        j.stringLiteral((path.node?.callee as CodeModNode)?.property?.name as string),
      ]),
    )
}
