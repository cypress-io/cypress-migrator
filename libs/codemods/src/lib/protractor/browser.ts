  ...
  return nodes
    .filter((path: any) => {
      ...
      return (
        ...
        unsupportedBrowserMethods.includes(path.node.callee.property.name)
      );
    })
    .forEach(() => {
      return (path: any) =>
...
export function transformBrowserMethods(j: JSCodeshift, nodes: any): any {
  return nodes
    .filter((path: any) => {
      ...
      return (
        identifier?.name === 'browser' &&
        supportedBrowserMethods.includes(path.node.callee.property.name)
      );
    })
    .forEach((path: any) => {
      ...
      if (method === 'perform') {
        const propertyName =
          parentPath.value.expression.callee.object.callee.property.name;
            ...
            j.memberExpression(
              ...
              j.identifier(
                supportedBrowserMethods.includes(propertyName)
                  ? browserMethodTransforms[propertyName]
                  : propertyName
              ),
              false
            ),
      ...
      return j(path).replaceWith(
        replaceCySelector(
          j,
          method === 'findElement' ? node.arguments[0] : path.value,
          replacedMethod
        )
      );
    });
...
export function transformBrowserNavigate(j: JSCodeshift, root: any): any {
  return root
    ...
    .replaceWith((path: any) =>
      j.callExpression(
        j.memberExpression(j.identifier('cy'), j.identifier('go'), false),
